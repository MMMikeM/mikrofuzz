...
describe("field specs", () => {
	...
	it("penalty is added to the field's score and to the min", () => {
		const posts: Post[] = [{ title: "cat", body: "cat" }];
		const r = createFuzzySearch(posts, [
			{ text: (p) => p.title },
			{ text: (p) => p.body, penalty: 2 },
		])("cat")[0]!;
		expect(r.fields[0]?.score).toBe(0); // exact
		expect(r.fields[1]?.score).toBe(2); // exact 0 + penalty 2
		expect(r.fields[1]?.tier).toBe("exact"); // tier is the raw match tier
		expect(r.score).toBe(0);
	});

	it("penalty keeps body from outranking title", () => {
		const posts: Post[] = [
		...
		const fields = [
			{ text: (p: Post) => p.title },
			{ text: (p: Post) => p.body, penalty: SCORES.CONTAINS },
		];
		expect(createFuzzySearch(posts, fields)("redux")[0]!.item.title).toBe("Redux Toolkit stuff");
		// without the penalty, the body match wins
		const noPenalty = fields.map((f) => ({ text: f.text }));
		expect(createFuzzySearch(posts, noPenalty)("redux")[0]!.item.title).toBe("unrelated title");
	});

	it("tolerates null field texts", () => {
		const posts: Post[] = [{ title: "hello", body: "world" }];
		const r = createFuzzySearch(posts, [{ text: () => null }, { text: (p) => p.title }])("hello")[0]!;
...
describe("FuzzyResult invariants", () => {
	const search = createFuzzySearch([{ a: "hello world", b: "xyz" }], [
		{ text: (i) => i.a },
		{ text: (i) => i.b },
	]);
	...
	it("score equals the minimum of the matched fields' scores", () => {
		const r = createFuzzySearch([{ a: "pineapple", b: "app store" }], [
			{ text: (i) => i.a },
			{ text: (i) => i.b },
		])("app")[0]!;
